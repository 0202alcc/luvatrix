...
This runbook bootstraps the Luvatrix server structure defined in `discord/discord.md` using:
- `discord/ops/blueprint.json`
- `discord/scripts/bootstrap_discord.py`
...
python discord/scripts/bootstrap_discord.py
```
...
export DISCORD_BOT_TOKEN='YOUR_BOT_TOKEN'
export DISCORD_GUILD_ID='YOUR_GUILD_ID'
python discord/scripts/bootstrap_discord.py --apply
```

What this automates:
1. Creates required roles from `discord/discord.md`.
2. Creates all categories/channels from `discord/discord.md`.
3. Applies restricted visibility for `05_EXECUTIVE` (CEO + Leads only).
4. Posts and pins starter messages/templates in key channels.
...
2. Paste current non-private artifacts into:
- `#project-charter-public`
- `#workplan-public`
- `#reports-index-public`
3. Confirm `#onboarding-checklist` and `#agent-onboarding` are pinned.
4. Confirm `05_EXECUTIVE` is hidden from non-lead roles.
5. Run one onboarding simulation with a new member or AI agent.
...
export DISCORD_BOT_TOKEN='YOUR_BOT_TOKEN'
export DISCORD_GUILD_ID='YOUR_GUILD_ID'
./discord/scripts/check_discord_bot_rollout.sh
```
...
5. Verify `05_EXECUTIVE` visibility is still restricted to `CEO` and `Leads`.
...
3. Update `discord/ops/blueprint.json` when server structure evolves.
