...
class PlanesRuntimeTests(unittest.TestCase):
    ...
    def test_drag_adaptive_quality_reduces_button_shader_cost_during_active_drag(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(bool(drag_button.backdrop_cache_enabled))
            self.assertGreaterEqual(float(drag_button.roi_inset_px), 2.0)
            self.assertEqual(int(drag_button.downsample_factor), 2)
            perf = app.state.get("perf", {})
    ...
    def test_plane_runtime_exposes_frame_timing_stages_and_event_counters(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertGreaterEqual(int(perf.get("backdrop_cache_hits", 0)), 0)
            self.assertGreaterEqual(int(perf.get("backdrop_cache_misses", 0)), 0)
            self.assertGreaterEqual(int(perf.get("backdrop_cache_entry_count", 0)), 0)
