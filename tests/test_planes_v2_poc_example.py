...
class PlanesV2PocExampleTests(unittest.TestCase):
    def test_planes_poc_compiles_and_inherits_web_metadata(self) -> None:
        app = create()
        self.assertEqual(app.metadata.title, "Planes v2 Web Scroll + Section Cut Demo")
        self.assertEqual(app.metadata.tab_title, "Planes v2 Web Scroll + Section Cut Demo")
