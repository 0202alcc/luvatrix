# Planning Operator Cheatsheet (GateFlow 1.0.0)
...
1. Wrapper default is pinned to `gateflow==1.0.0`.
...
```bash
uvx --from gateflow==1.0.0 gateflow --root . init
...
```bash
uvx --from gateflow==1.0.0 gateflow --root . api GET /milestones
uvx --from gateflow==1.0.0 gateflow --root . api GET /tasks
uvx --from gateflow==1.0.0 gateflow --root . api GET /boards
uvx --from gateflow==1.0.0 gateflow --root . api GET /frameworks
uvx --from gateflow==1.0.0 gateflow --root . api GET /backlog
```

## 3) CRUD (resource/API shim)

```bash
uvx --from gateflow==1.0.0 gateflow --root . milestones create --body '{"id":"A-021", ...}'
uvx --from gateflow==1.0.0 gateflow --root . tasks create --body '{"id":"T-1201", ...}'
uvx --from gateflow==1.0.0 gateflow --root . tasks update T-1201 --body '{"status":"Success Criteria Spec"}'
uvx --from gateflow==1.0.0 gateflow --root . boards create --body '{"id":"milestone:A-021", ...}'
```
...
```bash
uvx --from gateflow==1.0.0 gateflow --root . close task T-1201 --heads-up "Go: required checks passed on main"
uvx --from gateflow==1.0.0 gateflow --root . close milestone A-021 --heads-up "No-Go: missing deterministic replay evidence"
```
