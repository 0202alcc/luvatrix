#!/usr/bin/env python3
"""Planning domain mutation and validation services."""
...
def validate_closeout_packet(milestone_id: str, *, closeout_dir: Path) -> None:
...
def parse_body(*, body: str | None, body_file: str | None) -> dict[str, Any]:
...
def create_milestone(
    ...
    *,
    ...
    if body.get("status") == "Complete":
        validate_closeout_packet(mid, closeout_dir=closeout_dir)
...
def patch_milestone(
    ...
    *,
    ...
    if old_status != "Complete" and row.get("status") == "Complete":
        validate_closeout_packet(milestone_id, closeout_dir=closeout_dir)
...
def mutate_resource(
    *,
    ...
    closeout_dir: Path,
    force: bool = False,
    force_remove_deps: bool = False,
    reestimate_cost: bool = False,
    force_with_reason: str | None = None,
) -> str:
    all_task_ids = {t["id"] for t in tasks_master.get("tasks", [])} | {
    ...
    if resource == "milestones":
        if method == "POST":
            ...
            return create_milestone(schedule, body, all_task_ids, closeout_dir=closeout_dir)
        if method == "PATCH":
            if ident is None:
                raise ApiError("PATCH /milestones/{id} requires id in path")
            return patch_milestone(schedule, ident, body, all_task_ids, closeout_dir=closeout_dir)
        if method == "DELETE":
            if ident is None:
                raise ApiError("DELETE /milestones/{id} requires id in path")
            return delete_milestone(schedule, tasks_master, ident, force=force)
    ...
    if resource == "tasks":
        if method == "POST":
            ...
            return create_task(
                ...
                body,
                all_task_ids,
                reestimate_cost=reestimate_cost,
            )
        if method == "PATCH":
            if ident is None:
        ...
        if method == "DELETE":
            ...
            return delete_task(tasks_master, tasks_archived, ident, force_remove_deps)
    ...
    if resource == "backlog":
        if method == "POST":
            ...
            return create_backlog_item(backlog, body, milestone_ids, all_task_ids)
        if method == "PATCH":
            if ident is None:
                raise ApiError("PATCH /backlog/{id} requires id in path")
            return patch_backlog_item(backlog, ident, body, milestone_ids, all_task_ids)
        if method == "DELETE":
            if ident is None:
                raise ApiError("DELETE /backlog/{id} requires id in path")
            return delete_backlog_item(backlog, ident)
        raise ApiError(f"unsupported method for backlog: {method}")

    if method == "POST":
        if ident is None:
            raise ApiError("POST /frameworks/{name} requires name in path")
        return create_framework(boards, ident, body)
    if method == "PATCH":
        if ident is None:
            ...
            validate_board_definitions(boards)
            return f"updated default framework {boards['default_framework_template']}"
        return patch_framework(boards, ident, body)
    if method == "DELETE":
        if ident is None:
            raise ApiError("DELETE /frameworks/{name} requires name in path")
        return delete_framework(boards, ident)
